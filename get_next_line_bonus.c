...
/*   Updated: 2023/07/15 21:10:57 by msaez-lo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!var[fd])
		var[fd] = "";
	if (ft_strchrlen(var[fd], '\n') != -1)
	{
		ret = ft_strndup(var[fd], ft_strchrlen(var[fd], '\n') + 1);
		if (!ret)
			return (0);
		var[fd] = ln_in_var(var[fd], ft_strchrlen(var[fd], '\n')); //MALLOC SIN PROTEGER;
		return (ret);
	}
	ret = until_n(fd, var[fd]);
	if (!ret)
		return (0);
	auxch = var[fd];
	ret = clean(ret, &auxch, &(var[fd]));
	if (!ret)
		return (0);
	free(auxch);
	return (ret);
